...
const productData = {
  rings: [ { name: "9CT WHITE GOLD 0.33CT DIAMOND ETERNITY", price: 2799, image: chain1, badge: "Sale", rating: "★★★★★", count: 1 },
...
const TopProducts = () => {
  ...
  const renderProductGrid = (products) => (
    <div className={`d-flex flex-row flex-nowrap overflow-auto py-3 ${styles.shopGrid}`}>
      {products.map((product, index) => (
        <div key={index} className={styles.shopCardWrapper}>
            ...
            <div className={styles.imgBox}>
                <Card.Img className={`${styles.productImg} ${styles.default}`} src={product.image} alt={product.name} />
                <Card.Img className={`${styles.productImg} ${styles.hover}`} src={product.image} alt={`${product.name} alt`} />
            </div>
            <Card.Body className="d-flex flex-column text-center">
                <Card.Title as="p" className={styles.prodTitle}>{product.name}</Card.Title>
                <p className={styles.rating}>{product.rating} <span className={styles.count}>({product.count})</span></p>
                <p className={styles.price}>₹{product.price.toLocaleString()}</p>
                <div className={`${styles.actions} mt-auto`}>
                  <Button className={styles.btnCart} onClick={(e) => handleAddToCart(e, product)}>Add to Cart</Button>
                  <Button className={styles.btnLike} onClick={(e) => handleLike(e, product)}><i className="far fa-heart"></i></Button>
                </div>
            </Card.Body>
            </Card>
        </div>
